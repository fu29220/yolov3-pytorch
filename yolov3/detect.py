...
if __name__ == "__main__":
    ...
    for batch_i, (img_paths, input_imgs) in enumerate(dataloader):
        ...
        num_samples = input_imgs.size(0)
        img_dim = input_imgs.size(2)

        # Get detections
        with torch.no_grad():
            y = model(input_imgs.cuda())
            ### 
            outputs=[]
            num_classes = y[0].size(-1)-5
            for i, output in enumerate(y):
                ...
                output = torch.cat([pred_boxes.reshape(num_samples, -1, 4) * stride, pred_conf.reshape(num_samples, -1, 1),pred_cls.reshape(num_samples, -1, num_classes)],-1)
